...
function message(msg){
  var c = document.getElementById("entries-status");
...
task["update-siteinfo"] = function(data){
  ...
  safari.self.tab.dispatchMessage('message', {task: 'siteinfo'});
};
...
function request_full_story() {
  ...
  safari.self.tab.dispatchMessage('message', {task: 'fullfeed', url: lastItem.url});
}
  ...
  try {
    ...
    [
      ...
      if (url.match(i)) {
        ...
        safari.self.tab.dispatchMessage('message', {task: 'expand', url: url});
        throw "nothing to do...";
      }
    });
    ...
    for (var n = 0; n < siteinfos.length; n++) {
      try {
        if ((new RegExp(siteinfos[n].url)).test(url)) {
          ...
          icon.innerHTML = '<img src="' + safari.extension.baseURI + 'btn.png" class="fullfeed-btn"/>';
...
setTimeout(function() {
  ...
  document.addEventListener('keydown', function(e){
    if(e.target.tagName.toLowerCase() == "input" || e.target.tagName.toLowerCase() == "textarea"){
      ...
      var exp = 'id("current-entry")//a[contains(concat(" ", @class, " "), " entry-title-link ")]';
      var url =  $X(exp)[0].href;
      safari.self.tab.dispatchMessage('message', {task: 'open', url: url});
